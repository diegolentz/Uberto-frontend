...
export const CardFriends = ({ isFriend, friendData, id}: { isFriend: boolean, friendData: Friends, id:number}) => {
    return (
        <Box sx={{ width: '85%', flexDirection: 'row', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem', border: '1.5px solid #a737fc', borderRadius: '1rem' , }}>
            ...
            <Box width={'20%'}>
                {/* eliminar o agregar */}
                {isFriend ? (
                    <Button sx={{ color: 'red', borderColor: 'red', minWidth: '3rem', minHeight: '3rem', padding: '4px' }} onClick={() => { passengerService.removeFriend(id, friendData.id) }}>
